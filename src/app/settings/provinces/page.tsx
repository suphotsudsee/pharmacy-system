...
export default function ProvincesPage() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('ต้องการลบจังหวัดนี้ใช่หรือไม่?')) return;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold dark:text-white">🗺️ จัดการจังหวัด</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">จัดการข้อมูลจังหวัดและภูมิภาค</p>
        </div>
        {isAuthenticated && (
          <Link
            href="/settings/provinces/new"
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-300">ชื่อจังหวัด</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-300">ภูมิภาค</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredProvinces.map((province) => (
                <tr key={province.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{province.code}</td>
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{province.name}</td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{province.region || '-'}</td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{province._count.hospitals}</td>
                  {isAuthenticated && (
                    <td className="px-4 py-3 text-right">
                      <Link
                        href={`/settings/provinces/${province.id}`}
